...
import { applyPurge } from './cleanup-legacy-local-runs/apply.mjs'
import { buildEntries, classify, compareRunIds, residueCategories, terminalStatuses } from './cleanup-legacy-local-runs/classification.mjs'
import { writeInventoryManifest } from './cleanup-legacy-local-runs/inventory.mjs'
const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..')
const innerFlag = '--__cocoder-cleanup-legacy-local-runs-inner'
if (!process.argv.includes(innerFlag)) {
  const childArgs = ['exec', 'tsx', fileURLToPath(import.meta.url), innerFlag]
  if (process.argv.includes('--apply')) childArgs.push('--apply')
...
async function main() {
  ...
  const manifestPrefix = mode === 'apply' ? 'purge' : 'inventory'
  const manifestPath = join(localRoot, 'cleanup-legacy-local-runs', `${manifestPrefix}-${generatedAt.replace(/[:.]/g, '-')}.json`)
  ...
  const entries = await buildEntries({ rows, dirs, runsRoot, workspaceById, projectionChecker, core })
