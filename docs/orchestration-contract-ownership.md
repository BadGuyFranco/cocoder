...
| Persona/shared-standards rules stranded as side-channel memory | `packages/personas/base/shared-standards.md` owns portable cross-persona standards; role-specific portable rules belong in the relevant base persona; repo-specific rules belong in `cocoder/personas/deltas/` or `cocoder/standards/`. | Shared standards already require owner maps and aligned consumers for orchestration behavior (`packages/personas/base/shared-standards.md:66-75`). Ticket `0005` carries five concrete rules that it says still live outside their rightful governed files (`cocoder/tickets/open/0005-persona-file-memory-migrations.md:13-18`, `cocoder/tickets/open/0005-persona-file-memory-migrations.md:21-60`). Ticket `0017` carries the founder-brief single-source rule in a ticket/docs explainer rather than the governed standard (`cocoder/tickets/open/0017-promote-founder-brief-single-source-rule-to-shared-standards.md:23-40`). Base-persona tests only pin the current general durable-orchestration rule, not the 0017 specialization (`packages/personas/tests/base-personas.test.ts:59-68`). | `drifting`. The rules are known, portable, and named to rightful files, but still carried in tickets/explainers rather than the governed prompt files the runtime prepends (`packages/daemon/src/launcher.ts:98-120`). |
| Generated-vs-source UI artifact contract (`packages/ui/design-ref` vs `packages/ui/app`) | Founder decision needed between two possible owners: either `packages/ui/app` is the maintained source and `packages/ui/design-ref` becomes historical, or `packages/ui/design-ref` is made the source and fixes are folded back before generation. | `packages/ui/design-ref/README.md` still says the prototype is the source of truth for what to build (`packages/ui/design-ref/README.md:7-15`) and a binding implementation spec (`packages/ui/design-ref/README.md:30-35`). App code says it was rebuilt/ported from the design prototype (`packages/ui/app/App.tsx:1-5`, `packages/ui/app/model.ts:1-4`, `packages/ui/app/styles/fusion.css:1-7`). Ticket `0012` records that fixes were reverted by regenerating from a frozen design-ref snapshot and says the risk remains live (`cocoder/tickets/open/0012-design-ref-rebuild-clobber-guard.md:13-24`). | `drifting`. Two trees still present themselves as live truth, and ticket `0012` explicitly says to pick one direction and enforce it (`cocoder/tickets/open/0012-design-ref-rebuild-clobber-guard.md:26-42`). |
| Ticket authoring and loader format | Core ticket helpers own the machine format: `composeTicketMarkdown`, `loadTicket`, and `readTickets`. Authoring surfaces must call or round-trip through those helpers. | `composeTicketMarkdown` emits full YAML frontmatter (`packages/core/src/tickets/compose.ts:1-13`); `loadTicket` tolerates frontmatter-less historical tickets, validates frontmatter id when present, and warns on malformed ticket files instead of silently dropping them (`packages/core/src/tickets/loader.ts:40-47`, `packages/core/src/tickets/loader.ts:53-73`, `packages/core/src/tickets/loader.ts:84-94`); dashboard ticket creation composes, parses, round-trips, indexes, and commits tickets through the same path (`packages/daemon/src/routes.ts:611-649`, `packages/daemon/src/routes.ts:872-882`); the create-ticket Play says to use `composeTicketMarkdown` and validate with `loadTicket`/`readTickets` (`packages/personas/base/plays/create-ticket.md:13-16`, `packages/personas/base/plays/create-ticket.md:32-42`). | `already-fixed`. Ticket `0015` described the old silent-drop bug (`cocoder/tickets/open/0015-tickets-silently-dropped-without-frontmatter.md:28-38`), but current tests prove frontmatter-less tickets load, malformed tickets warn, and real fallback tickets `0009`/`0011`/`0014` are present (`packages/core/tests/tickets.test.ts:55-120`); authoring-play tests prove create-ticket commits a valid ticket and index row (`packages/daemon/tests/authoring-play.test.ts:86-144`). |
...
- `0005` - `folded-in`. It is the side-channel-memory flavour named by this priority
  (`cocoder/priorities/founder-brief-format-durability.md:61-62`), and it carries concrete rules whose
  rightful homes are persona/standards/governance files (`cocoder/tickets/open/0005-persona-file-memory-migrations.md:21-60`).
- `0012` - `folded-in`. It is the generated-vs-source flavour named by this priority
  (`cocoder/priorities/founder-brief-format-durability.md:63-64`); it remains a founder architecture call
  because the ticket asks to choose Option A or B before enforcement (`cocoder/tickets/open/0012-design-ref-rebuild-clobber-guard.md:26-42`).
- `0015` - `folded-in`, with the runtime defect already fixed. The priority names it as the
  ...
  The ticket file is still open, so the remaining governance action is closure/reconciliation, not another
  loader repair.
...
1. Move the 0017 founder-brief single-source specialization into `packages/personas/base/shared-standards.md` and pin it with a base-persona test so runtime validators/fallback emitters must parse or import governed owners instead of copying formats.
2. Apply ticket 0005's side-channel persona/standards rules to their named governed files, using the ADR-0012 portability test for base persona changes and closing only the migrated items.
3. Resolve ticket 0012 by making the founder's owner choice explicit, then enforce it: recommended path is to mark `packages/ui/design-ref` historical and add a guard preventing wholesale regeneration from clobbering `packages/ui/app`.
