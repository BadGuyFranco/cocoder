...
| Persona/shared-standards rules stranded as side-channel memory | `packages/personas/base/shared-standards.md` owns portable cross-persona standards; role-specific portable rules belong in the relevant base persona; repo-specific rules belong in `cocoder/personas/deltas/` or `cocoder/standards/`. | Shared standards already require owner maps and aligned consumers for orchestration behavior (`packages/personas/base/shared-standards.md:66-75`). Ticket `0005` carries five concrete rules that it says still live outside their rightful governed files (`cocoder/tickets/open/0005-persona-file-memory-migrations.md:13-18`, `cocoder/tickets/open/0005-persona-file-memory-migrations.md:21-60`). Ticket `0017` carries the founder-brief single-source rule in a ticket/docs explainer rather than the governed standard (`cocoder/tickets/open/0017-promote-founder-brief-single-source-rule-to-shared-standards.md:23-40`). Base-persona tests only pin the current general durable-orchestration rule, not the 0017 specialization (`packages/personas/tests/base-personas.test.ts:59-68`). | `partially-fixed`. Ticket 0005 items 3-5 (portable) are migrated to the governed base files and base-persona-test-pinned (`d06ae45`); 0017 was closed by `aa7addc`. Items 1-2 are repo-specific (`cocoder/personas/deltas/oscar.md`, `cocoder/AGENTS.md`) and deferred — outside run_148's Oscar support write-scope; ticket 0005 stays open for them. |
...
2. **Portable parts done** (`d06ae45`). Ticket 0005 items 3-5 (adversarial plan review, design-seam discussion style, launchability/green-claims) are applied to `oscar.md`/`shared-standards.md`/`bob.md`, ADR-0012-portable, base-persona-test-pinned. **Deferred:** items 1-2 are repo-specific (`cocoder/personas/deltas/oscar.md` daemon-launch delta, `cocoder/AGENTS.md` name disambiguation) and fall outside run_148's Oscar support write-scope; ticket 0005 stays open for them.
