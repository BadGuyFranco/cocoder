...
| Target | Meaning |
|---|---|
| `cocoder-product` | A product change to CoCoder itself: `packages/**`, `templates/**`, public `docs/**`, schemas, shipped prompts, or `packages/personas/base/**`. Contributor-only product work, routed by the run's write-scope and surfaced by the spine receipt when files are out-of-lane; write-scope is advisory and does not withhold commits (no separate developer-mode toggle in v2); in the dogfood it is the ADR-0012 portability-test call |
