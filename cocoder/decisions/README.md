...
| [0013](./0013-orchestration-observation.md) | Orchestration + observation: Oscar drives Bob through a multi-atom plan; tiered continuous monitoring (Oscar→Bob, Deb→Oscar, Oz→sessions) with the direct-your-primary rule (refines 0004) — *amended by [0046](./0046-run-tests-required-checkpoint.md): code-touching atoms require a green run-tests result as a required verify-gate input before commit* | Accepted |
| [0014](./0014-living-adrs.md) | ADRs are living documents (founder-approved, conflict-audited) | Accepted |
| [0016](./0016-deb-scoped-repair-fallback.md) | Deb: the scoped CoCoder repair fallback — live status feed + nudge-request channel + gate-enforced repair mode; base/delta write-scope split (refines 0013; repairs land via the spine, ADR-0023) — *amended by [0040](./0040-oz-write-side-autonomy.md): adds a separate proactive Oz `oz-action` self-direct write lane distinct from Deb's reactive repair* | Accepted |
...
| [0023](./0023-workspace-commit-spine.md) | **The workspace commit spine: direct-to-branch, single mode** — one commit service for all actors; active checkout only; collapses the three divergent commit paths; dissolves the run-branch strand class (supersedes 0015/0021/0022, reconciles 0007) — *amended by [0046](./0046-run-tests-required-checkpoint.md): the one verify gate takes green run-tests as a required input for code-touching atoms; not a second commit lane* | Accepted |
...
| [0038](./0038-priority-visibility-invariant.md) | **Priority visibility invariant** — priority write spines register `order.json` by construction in the same governance commit; `findOrphanedPriorities` remains a backstop while `readPriorities` keeps ADR-0010 append behavior | Accepted (2026-06-23) |
...
| [0040](./0040-oz-write-side-autonomy.md) | **Oz write-side autonomy** — a narrow self-direct `oz-action` write lane lets Oz make reversible edits to existing governance (reorder priorities, open/close tickets, narrow doc fixes, non-Objective priority edits) with the daemon gate-committing only allowed paths and holding the rest back; conversational authoring still routes net-new Objectives through founder approval via the `author` spine; product code, secrets, install-local state, and lifecycle stay forbidden (amends 0016/0017/0025; builds on 0010/0035/0023; layers on the `oz-file-access` read foundation) | Accepted (2026-06-23) |
| [0046](./0046-run-tests-required-checkpoint.md) | **run-tests is a required checkpoint for code-touching atoms** — green run-tests feeds the single existing verify gate as a required input (not a second commit lane); inherited via base standards; advisory degrade when no test surface; binds agents, never founder direct work (amends 0013/0023; reconciles 0028/0029/0033) | Accepted (2026-06-29) |
| [0047](./0047-unit-test-architecture-standard.md) | **Unit-test architecture standard** — ratifies `cocoder/standards/test-architecture.md` as the single operational owner: one behavior area per `*.test.ts`, one `*-support.ts` fixture owner per subsystem, soft ~600-line/25-test split budget, tests next to the code (never under `cocoder/`), convergence-target (not forced migration) for managed repos; worked example is the runner.test.ts split (builds on 0033/0046) | Accepted (2026-06-30) |
