...
- **Hardening (run_251):** real `runStandalone`→`runRun` integration test without runner injection; daemon-up fail-fast (or `--force`) before opening the live store on non-isolated runs. Model-resolution semantics (most-capable-by-curation + force-latest override) remain a founder confirmation gate before archive.
