...
- **Rebuild-clobber (failure-catalog F21):** #2/#5/#7/#8 were fixed 2026-06-14, then silently reverted when the "Fusion" renderer rebuild (`2ccff89`) regenerated `packages/ui/app` from the frozen `design-ref/`. run_94 re-fixed them. `design-ref/` still holds the old `claude-code` id → **ticket 0012** (design-ref rebuild guard).
