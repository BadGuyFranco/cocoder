...
- Live observation harness: `scripts/observe-retention-live.mjs` — read-only snapshot/diff for developer/Oscar verification; founder enablement goes through Oz chat `retention enable [N]` (persists via daemon settings spine, runs GC on enable, surfaces footprint delta + pruned/protected runs + retention-gc audit entry in chat).
...
**Known soft spots:**
- **Founder affordance for live enablement shipped (0083, run_283).** Oz chat `retention enable [N]` replaces hand-edits and node commands; `observe-retention-live.mjs` remains developer/Oscar-only.
...
5. **Live enablement — observed (2026-06-28).** Live daemon-boot GC pass confirmed (run_279/137 via governed settings); legacy residue handled separately ([0082](../tickets/closed/0082-cleanup-legacy-unprojected-and-unresolvable-workspace-local-runs.md), closed run_282). Founder re-enablement now routes through Oz chat `retention enable [N]` ([0083](../tickets/closed/0083-retention-live-enablement-has-no-founder-facing-affordance.md), closed run_283) — no settings.json hand-edit or node commands.
6. **Archived (2026-06-28).** Objective met and proven on a live install; reply `archive` only applies to other priorities still awaiting confirmation.
