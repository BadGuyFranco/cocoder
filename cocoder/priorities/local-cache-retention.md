...
5. **Live enablement — founder-approved (2026-06-28); the next run drives it.** The integration proof is clean, and the founder has approved live enablement. The next run sets `retention.enabled: true` in `local/settings.json` as the now-approved (no longer silent) step, boots/Refreshes the daemon, and observes one live GC pass — capturing before/after `local/` footprint, the audit-log GC entry, and that no non-terminal/protected run was pruned. Still do not restart the daemon by killing processes; use the sanctioned Refresh/boot affordance.
6. **Archive only once effectiveness is observed on a live install**, not merely unit-proven or scratch-proven.

**Disposition: `blocked` → next run is a scoped live-enablement + observe-then-archive run (founder decisions resolved 2026-06-28).** Run_136 landed the integration-seam harness and verified retention through the real daemon boot path on a scratch copy of this install's live store. Both founder beats are now decided: (1) **live enablement approved** — the next run performs it and observes one live GC pass; (2) **legacy residue accepted** as out-of-engine-scope and tracked in ticket [0082](../tickets/open/0082-cleanup-legacy-unprojected-and-unresolvable-workspace-local-runs.md) (retention bounds projected, resolvable runs; the pre-run_138 + unresolvable-workspace residue is a separate one-time cleanup). No further build is needed before that run; archive follows live effectiveness, not scratch proof alone.
