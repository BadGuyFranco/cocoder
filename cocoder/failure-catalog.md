...
| F21 | **Renderer rebuild silently reverts committed bug fixes** (run_94, 2026-06-15). Six dashboard bugs were fixed + committed on 2026-06-14, but a later wholesale *"rebuild the renderer against the V1 design (Fusion)"* commit (`2ccff89`) plus the isolation-lane change regenerated `packages/ui/app` from the frozen `packages/ui/design-ref/` snapshot — which never held the fixes — silently reverting #2/#5/#7/#8 (verbose rows, wrong persona order, "Sub-agents" label, stale banners) while #9 survived. The "already-fixed" bugs presented as open; run_94 spent two atoms RE-fixing them. Risk still live: `design-ref/` retains the old `claude-code` CLI id, so run_94's #11 `claude` rename + the relabels are exposed to the next rebuild. | `design-ref/` is treated as a live regeneration SOURCE OF TRUTH, but hand-applied fixes to the generated tree (`packages/ui/app`) are never folded back into it — so any regenerate-from-design-ref clobbers them. The F1/F4 single-source class, applied to generated-vs-source UI code. | A generated artifact and its source need ONE direction of truth: either fixes land in `design-ref/` and the app regenerates from it, or `design-ref/` is retired as a live regeneration source once the app diverges. Never regenerate a hand-maintained tree from a frozen snapshot. **Follow-up:** ticket 0012 — a guard/diff-gate that flags when a design-ref rebuild would revert committed `packages/ui/app` changes (resolved: 0012 closed, Option A — `design-ref/` marked historical and enforcer-guarded). |
...
- **F23 — Test fixture drifted from the owner contract and silently reddened a consuming suite
  ...
  ticket [0022](./tickets/closed/0022-wrap-up-contract-fixture-drift-daemon-suite.md) (closed).
