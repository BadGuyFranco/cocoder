...
**Active task (Sub-Playbook A audit §4 port-first list E2.2e.10, via Sub-Playbook E's proven orchestration loop):** Talia ports `session-wrap.test.mjs` from CoBuilder into `packages/core/tests/session-wrap.test.mjs`. E2.2e.1–E2.2e.9 closed 2026-05-22. Full suite is now 219/219. This is the next file per audit order. **Source:** `/Volumes/NAS LOCAL/CoBuilder/infrastructure/cobuilder-build/orchestration/tests/session-wrap.test.mjs` (310 lines). **Target:** `packages/core/tests/session-wrap.test.mjs`. **Allowed write zone:** `packages/core/tests/**` only. **Translation rules:** swap CoBuilder paths (`cobuilder-build/orchestration/`) for CoCoder paths (`packages/core/`); preserve test names + assertions verbatim; use `node:test` + `node:assert/strict`. If a test depends on git state, wrap it in a temporary git fixture under `packages/core/tests/` the same way `core.test.mjs` does. **Coverage focus:** wrap audit, handoff consistency. **Schemas-dist mtime caveat** still applies (`PORT-NOTES.md` finding F). **Done = `pnpm -F core test session-wrap` green; no mutation outside `packages/core/tests/`.**
