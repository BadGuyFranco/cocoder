...
**Active task (Sub-Playbook A audit §4 port-first list E2.2e.6, via Sub-Playbook E's proven orchestration loop):** Talia ports `orchestrator-commit.test.mjs` from CoBuilder into `packages/core/tests/orchestrator-commit.test.mjs`. E2.2e.1–E2.2e.5 closed 2026-05-22 (the launch.test.mjs port at E2.2e.5 surfaced + fixed 4 product-code bugs; full suite is now 165/165). This is the next file per audit order. **Source:** `/Volumes/NAS LOCAL/CoBuilder/infrastructure/cobuilder-build/orchestration/tests/orchestrator-commit.test.mjs` (708 lines). **Target:** `packages/core/tests/orchestrator-commit.test.mjs`. **Allowed write zone:** `packages/core/tests/**` only. **Translation rules:** swap CoBuilder paths (`cobuilder-build/orchestration/`) for CoCoder paths (`packages/core/`); preserve test names + assertions verbatim; use `node:test` + `node:assert/strict`. If a test depends on git state, wrap it in a temporary git fixture under `packages/core/tests/` the same way `core.test.mjs` does. **Coverage focus:** route-owned commits, `filesChanged` guards, verification-artifact guard, and the M4.22 `--developer-mode` cocoder-product write belt added 2026-05-22. **Schemas-dist mtime caveat** still applies (`PORT-NOTES.md` finding F). **Done = `pnpm -F core test orchestrator-commit` green; no mutation outside `packages/core/tests/`.**
