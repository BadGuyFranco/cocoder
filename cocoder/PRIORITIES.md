...
**Active task (Sub-Playbook A audit §4 port-first list E2.2e.11, via Sub-Playbook E's proven orchestration loop):** Talia ports `repo-state.test.mjs` from CoBuilder into `packages/core/tests/repo-state.test.mjs`. E2.2e.1–E2.2e.10 closed 2026-05-22. Full suite is now 228/228. This is the next file per audit order — smallest port in the §4 list. **Source:** `/Volumes/NAS LOCAL/CoBuilder/infrastructure/cobuilder-build/orchestration/tests/repo-state.test.mjs` (50 lines). **Target:** `packages/core/tests/repo-state.test.mjs`. **Allowed write zone:** `packages/core/tests/**` only. **Translation rules:** swap CoBuilder paths (`cobuilder-build/orchestration/`) for CoCoder paths (`packages/core/`); preserve test names + assertions verbatim; use `node:test` + `node:assert/strict`. **Coverage focus:** add-lanes repo audit. **Note:** the `DURABLE_ORCHESTRATION_PREFIXES` constant was extended from `['cocoder/']` to `['cocoder/', 'packages/core/']` in PR #3 — tests that previously asserted the single-prefix behavior may need to adapt to reflect the new dual-prefix coverage. **Done = `pnpm -F core test repo-state` green; no mutation outside `packages/core/tests/`.**
