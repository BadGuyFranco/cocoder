...
**Active task (Sub-Playbook A audit §4 port-first list E2.2e.9, via Sub-Playbook E's proven orchestration loop):** Talia ports `lead-rescue.test.mjs` from CoBuilder into `packages/core/tests/lead-rescue.test.mjs`, **AND** ports the supporting fixture from `tests/fixtures/lead-rescue/valid-supersession-record.json` into `packages/core/tests/fixtures/lead-rescue/valid-supersession-record.json`. E2.2e.1–E2.2e.8 closed 2026-05-22. Full suite is now 208/208. This is the next file per audit order. **Source test:** `/Volumes/NAS LOCAL/CoBuilder/infrastructure/cobuilder-build/orchestration/tests/lead-rescue.test.mjs` (511 lines). **Source fixture:** `/Volumes/NAS LOCAL/CoBuilder/infrastructure/cobuilder-build/orchestration/tests/fixtures/lead-rescue/valid-supersession-record.json`. **Target test:** `packages/core/tests/lead-rescue.test.mjs`. **Target fixture:** `packages/core/tests/fixtures/lead-rescue/valid-supersession-record.json`. **Allowed write zone:** `packages/core/tests/**` only (covers both target paths). **Translation rules:** swap CoBuilder paths (`cobuilder-build/orchestration/`) for CoCoder paths (`packages/core/`); preserve test names + assertions verbatim; use `node:test` + `node:assert/strict`; create the `fixtures/lead-rescue/` subdir if absent. **Coverage focus:** supersession records, finalize-with-dirty behavior. **Schemas-dist mtime caveat** still applies (`PORT-NOTES.md` finding F). **Done = `pnpm -F core test lead-rescue` green; both target files exist; no mutation outside `packages/core/tests/`.**
