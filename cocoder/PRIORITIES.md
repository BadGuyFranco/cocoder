...
**Active task (Sub-Playbook A audit §4 port-first list E2.2e.12, via Sub-Playbook E's proven orchestration loop — THE FINAL PORT IN THE §4 LIST):** Talia ports `launch-command.test.mjs` from CoBuilder into `packages/core/tests/launch-command.test.mjs`. E2.2e.1–E2.2e.11 closed 2026-05-22. Full suite is now 229/229. **Source:** `/Volumes/NAS LOCAL/CoBuilder/infrastructure/cobuilder-build/orchestration/tests/launch-command.test.mjs` (121 lines). **Target:** `packages/core/tests/launch-command.test.mjs`. **Allowed write zone:** `packages/core/tests/**` only. **Translation rules:** swap CoBuilder paths (`cobuilder-build/orchestration/`) for CoCoder paths (`packages/core/`); preserve test names + assertions verbatim; use `node:test` + `node:assert/strict`. **Coverage focus:** wrapper validity (this port was deferred per audit §4 "port AFTER M4.3 path rename" — M4.3 closed 2026-05-22 evening; this is the safe time to land). **After this lands, the entire audit §4 port-first list is closed** and the next priority is either Sub-Playbook A free-wins (M4.5–M4.14, M4.16–M4.21) or Sub-Playbook B (workspace template + `cocoder init`). **Done = `pnpm -F core test launch-command` green; no mutation outside `packages/core/tests/`.**
