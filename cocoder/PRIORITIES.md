...
**Active task (Sub-Playbook A audit §4 port-first list E2.2e.7, via Sub-Playbook E's proven orchestration loop):** Talia ports `debugger.test.mjs` from CoBuilder into `packages/core/tests/debugger.test.mjs`. E2.2e.1–E2.2e.6 closed 2026-05-22 (launch.test.mjs at E2.2e.5 surfaced + fixed 4 product-code bugs in PR #3; orchestrator-commit.test.mjs at E2.2e.6 surfaced + fixed 1 bounded-commit dirty-check bug in PR #7). Full suite is now 190/190. This is the next file per audit order. **Source:** `/Volumes/NAS LOCAL/CoBuilder/infrastructure/cobuilder-build/orchestration/tests/debugger.test.mjs` (498 lines). **Target:** `packages/core/tests/debugger.test.mjs`. **Allowed write zone:** `packages/core/tests/**` only. **Translation rules:** swap CoBuilder paths (`cobuilder-build/orchestration/`) for CoCoder paths (`packages/core/`); preserve test names + assertions verbatim; use `node:test` + `node:assert/strict`. If a test depends on git state, wrap it in a temporary git fixture under `packages/core/tests/` the same way `core.test.mjs` does. **Coverage focus:** prepare-debugger, evidence follow, pane/root checks. **Schemas-dist mtime caveat** still applies (`PORT-NOTES.md` finding F). **Done = `pnpm -F core test debugger` green; no mutation outside `packages/core/tests/`.**
