...
- [x] **M4.12** Port `packages/core/baselines/accepted-reference-baseline.md` per extraction manifest (audit §B2). `check-immutable-baseline` works with default args after. — *Closed 2026-05-23 (Group G): the upstream CoBuilder baseline shipped a pre-generated table for `cobuilder-build/build-personas/`, which doesn't map onto CoCoder's surface; "porting" means generating the equivalent against `cocoder/personas/` instead. Authored `packages/core/baselines/regenerate.mjs` as the source-of-truth regenerator + intent doc, then ran it to produce the initial frozen snapshot. `pnpm exec cocoder check-immutable-baseline` now returns `ok: true` (19 entries) with zero differences. Founder-approved persona-library changes regenerate + commit the baseline in the same PR.*
...
**M4 Checkpoint:** [x] All Free-wins (M4.1–M4.21) done; all Founder-gated tasks (M4.22–M4.27) either done or formally deferred via founder approval logged in Decision Log. — *Reached 2026-05-23: every M4 row ticked. Free-wins landed across Groups A–G (PRs #17, #18, #19, #20, #21, #22, #23, and this PR). Founder-gated tasks M4.22–M4.27 closed 2026-05-22 with regression tests. Suite green at every batch boundary.*
...
| Expand | M1: 7 · M2: 11 · M3: 6 · **M4: 27** | M1: 7, M2: 11 (E2.2e fully closed 2026-05-23; E2.4 closed by M4.4), M3: 6 (E3.5 re-ticked 2026-05-23 by M4.10), M4: 27 (ALL — M4 Checkpoint reached 2026-05-23) | **Active** |
...
| **Total** | **74** (was 42; M4 + audit additions) | **69** | **Active (Refine)** |
