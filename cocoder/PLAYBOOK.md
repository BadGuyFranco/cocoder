...
4. `new-primary-root` — **ACTIVE (run_128, 2026-06-17).** Onboard a primary root via the
   ...
   (run_111–128): Atoms F/1/5a/3/4 (run_111–112); run_123 landed the full P1 input layer; run_124
   landed executor P1 ACTION integration (`94de715`); run_125 landed P2a (`a47bd8b`); run_126 landed
   P2b (`66b5038`); run_127 landed P2c (`022d774`); **run_128 landed P3** (`775bf55`) —
   `p3-cross-check.ts`/`p3-input.ts`/`p3-action.ts`/`p3-render.ts` capped convergence loop (3/30min/
   min(125k,alloc), non-gameable ≥2-round predicate, ≤3 follow-up reads/round); launcher composes
   P1→P2→P3; daemon e2e proves resume→P2→real P3→P4 gate (core 322 + daemon 208 green). **Next:**
   **executor P4** (fresh dedicated session) — founder-question checkpoint ACTION integration
   (`p4-action.ts`, three question classes, e2e gate carries populated questions); then Atoms 9–11 +
   tech-stack-template build; live CoPublisher Takeover + dogfood Drift Audit proofs remain gated on
   executor end-to-end on fakes. **Absorbs `workspace-onboarding`.** Phase 5 ("first external repo").
