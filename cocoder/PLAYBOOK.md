...
## Priorities — where the live set lives

The live active set is the flat `*.md` files in [`cocoder/priorities/`](./priorities/);
the directory listing is the index (non-recursive scan). Ordering is the order-only runtime
overlay [`cocoder/priorities/order.json`](./priorities/order.json), per
[ADR-0035](./decisions/0035-priority-creation-always-placed-or-halted.md) and
[ADR-0038](./decisions/0038-priority-visibility-invariant.md). Deferred priorities live in
[`cocoder/priorities/backlog/`](./priorities/backlog/). Per-priority status/disposition is owned
by each Playbook file and the run pickup, not duplicated here. See
[`cocoder/priorities/AGENTS.md`](./priorities/AGENTS.md).
