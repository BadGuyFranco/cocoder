...
export function parseArgs(tokens) {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (!next || next.startsWith('--')) args[key] = 'true';
      ...
      if (['contract', 'prioritySlug', 'status', 'reason', 'summary', 'runId', 'jobId', 'sessionId', 'confirmRunId', 'mode', 'followIntervalSeconds', 'maxCycles', 'execute', 'executeService', 'deferStart', 'attach', 'stopTerminalSessions', 'founderApprovedTeardown', 'sessionLineLimit', 'owner', 'nonce', 'now', 'ttlMs', 'staleMs', 'timeoutMs', 'thresholdDays', 'maxChars', 'maxEntries', 'maxEntryLines', 'maxEntryChars', 'allowLive', 'cdpUrl', 'socketName', 'socketPath', 'lane', 'lanes', 'message', 'command', 'tmuxBin', 'noSession', 'gitWrite', 'supersededLane', 'resolvingLane', 'basis', 'findings', 'evidence', 'id', 'name', 'tmuxSocket', 'createdBy', 'personaPaths', 'sessionLog', 'topologyOption', 'requiredPersonas', 'autoAttachAddedLanes', 'workspaceSlug', 'developerMode', 'allowConcurrentPriorityRun', 'revealSecrets', 'service', 'executorCommand', 'model'].includes(key)) args[key] = next;
      index += 1;
    }
  }
  return args;
