const HELP_TEXT = `CoCoder orchestration core CLI
...
Commands:
  ...
  validate-orchestration-services [--services-dir PATH] [--contracts-dir PATH]
...
/** Primary command tokens listed in help (excludes alias notes and config subcommands). */
export function helpListedCommands() {
  return HELP_TEXT.split('\n')
    .map((line) => line.trim())
    .filter((line) => line && !line.startsWith('CoCoder') && !line.startsWith('Commands:'))
    .map((line) => line.split(/\s+/)[0])
    .filter((name) => name && name !== 'prepare-debug' && !name.startsWith('(') && name !== 'config');
}
