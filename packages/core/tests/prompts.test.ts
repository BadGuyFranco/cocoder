...
describe('buildBuilderDispatch', () => {
  ...
  test('founder-continue dispatch does not strand a parked run by asking Oscar to write the next directive', () => {
    ...
    expect(dispatch).toContain('Do not proactively write /runs/run_7/directive-2.json yourself now')
    expect(dispatch).toContain('founder-answer lane')
    expect(dispatch).toContain('founder-answer run_7 <answer>')
    expect(dispatch).toContain('cocoder oz founder-answer run_7 <answer>')
    expect(dispatch).toContain('Do not run `cocoder oz resume run_7` to continue after a founder decision')
    expect(dispatch).toContain('in these same live panes when they are still attached')
    expect(dispatch).toContain('resumes this held run')
    expect(dispatch).not.toContain('the Oz resume path')
    expect(dispatch).not.toContain('no live runner will be waiting')
