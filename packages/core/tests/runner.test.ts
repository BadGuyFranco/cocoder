...
const writeFounderStopSignal = async (runDir: string): Promise<void> => {
  ...
  await writeFile(founderStopSignalPath(runDir), `${JSON.stringify({ kind: 'founder-stop', recordedBy: 'bob', note: 'Founder explicitly said stop.' }, null, 2)}\n`, 'utf8')
}
...
describe('runRun (multi-atom loop)', () => {
  ...
  test('founder-stop artifact at the directive boundary holds without dispatch, abandon, quarantine, or nudge', async () => {
    ...
    expect(events.find((e) => e.type === 'run-held')?.data).toEqual({ park: 'pre-dispatch', atom: 0, directive: null })
  ...
  test('founder-stop artifact after a passing atom holds before the next-directive dispatch', async () => {
    ...
    expect(store.listEvents(result.runId).find((e) => e.type === 'run-held')?.data).toEqual({ park: 'pre-dispatch', atom: 1, directive: null })
