...
async function collectLaunchPreflight(repoRoot) {
  ...
  return {
    launcherSyntax: await runCommand('zsh', ['-n', path.join(repoRoot, 'cocoder/Launch-Orchestrator.command')], { maxBuffer: 256 * 1024 }),
    debuggerLauncherSyntax: await runCommand('zsh', ['-n', path.join(repoRoot, 'cocoder/ORCH DEBUGGER.command')], { maxBuffer: 256 * 1024 }),
    stopperSyntax: await runCommand('zsh', ['-n', path.join(repoRoot, 'cocoder/Stop-Orchestrator-Run.command')], { maxBuffer: 256 * 1024 }),
...
function renderDebuggerPrompt(bundle) {
  ...
  return [
    ...
    'debugger_git_authority: enabled by `ORCH DEBUGGER.command` via `COCODER_ORCH_DEBUGGER_GIT_WRITE=true`; Codex uses `danger-full-access` for founder-approved orchestration fixes and commits',
    '',
    '## Target Run Binding',
    '',
    bundle.noSession
    ...
    '',
    '- `ORCH DEBUGGER.command` always launches Codex with git authority enabled for founder-approved orchestration fixes and commits.',
    '- The launcher exports `COCODER_ORCH_DEBUGGER_GIT_WRITE=true`, so the wrapper uses Codex `danger-full-access` and approved debugger fixes can be committed after the required founder approval and verification gates.',
