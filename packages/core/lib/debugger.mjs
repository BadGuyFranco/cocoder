...
function renderDebuggerPrompt(bundle) {
  ...
  return [
    ...
    '',
    '- When an orchestration issue is recurring, mechanical, and bounded, consider whether it should become or use an orchestration service instead of staying as ad hoc debugger guidance.',
    '- Suggest or add a service when Oscar is spending expensive lead-model context on repeatable administrative work, when a failure has a recurring diagnostic shape, when wrap/teardown/continuation mechanics can be packetized, or when the same debugger finding appears across runs.',
    '- Do not suggest a service for priority ordering, architecture direction, founder-gated scope, persona dispatch judgment, Bob/Talia/Phil/Quinn/Ian domain work, or fuzzy work whose boundaries would hide judgment inside a cheap model.',
    '- New services are declaration files under `packages/core/services/<service-id>.json`, validated by `validate-orchestration-services`; runtime service code should not become a catalog god file.',
    '- Service declarations must keep Oscar as decision authority. Use read-only mode for audits and bounded-write mode only when `allowedWriteScopes` can be narrowed by each service packet.',
    '- Useful current services include `startup-context-audit` for oversized launch/handoff context, `handoff-compaction` for bloated status surfaces, and `wrap-execution` for Oscar-approved closeout edits.',
    '',
    '## No-Session Mode',
    '',
