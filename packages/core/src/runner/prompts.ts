...
export function buildNextOrWrapDispatch(nextDirectivePath: string, outcome: string): string {
  return `NEXT — ${outcome}. Write your next directive to ${nextDirectivePath}: delegate the next concrete in-priority atom by writing {"kind":"delegate","task":"…"} unless a real stop condition applies. If a founder decision is needed but concrete next work remains after the answer, write {"kind":"ask-founder-continue","question":"…"} so the runner surfaces the question without ending the run; then, after the answer is incorporated, write the following directive the runner requests. Otherwise write {"kind":"wrapup","pickup":"…"} to end the run with a resumable pickup brief. If you wrap, only write the directive file; do not also deliver a founder closeout in the pane. The runner will send a WRAP-UP READY artifact for exactly-once delivery after validation and landing outcome. Stop conditions are: priority done, no concrete next atom, different launch/surface needed, context genuinely tight, or failures/faults make continuing wasteful. If founder approval is the terminal stop condition, make that decision explicit in the pickup; the validated closeout must not say Founder Decision Needed is None. A clean commit boundary alone is not a reason to stop.`
}

export function buildFounderContinueDispatch(nextDirectivePath: string, question: string): string {
  return `FOUNDER DECISION NEEDED — surface this question to the founder without wrapping the run:\n\n${question}\n\nWhen the founder's answer is incorporated, write the next directive to ${nextDirectivePath}. If concrete in-priority work remains, write {"kind":"delegate","task":"…"} and include the answer in the task context. Only write {"kind":"wrapup","pickup":"…"} if the answer means the run should now end.`
}
