...
function readMemoryClaims(repoRoot: string, memoryDir: string): readonly DriftClaim[] {
  return ['codebase-map.md', 'tech-stack.md'].flatMap((name) => {
    ...
    return lines.flatMap((line, index) => {
      const seenSlugs = new Set<string>() // per-line slug guard: two same-line refs can't collide into a duplicate claim id
      return extractPathRefs(line, memoryDir, repoRoot).flatMap((ref) => {
        const refSlug = slug(ref)
        if (seenSlugs.has(refSlug)) return []
        seenSlugs.add(refSlug)
        return [claim(`memory:${source}:${index + 1}:${refSlug}`, 'memory', `${source} references ${ref}`, repoRoot, file, index + 1, [{ kind: 'path', value: ref }])]
      })
    })
  })
}
