...
| 8 | honor personas assignment `mode` | Personas: visible/headless toggle | `app/sections/Personas.tsx`; `PersonaAssignment` type in `electron/ipc-contract.ts` | **Partly served**: sub-agents are Play assignments over the existing `plays` map (ADR-0018); `mode` is still owed |
