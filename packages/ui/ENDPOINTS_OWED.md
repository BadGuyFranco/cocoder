...
| 8 | honor personas assignment `mode` | Personas: visible/headless toggle | `core/personas` validates `mode`; `plays/dispatch.ts` honors it for Play dispatch; UI `PersonaAssignment` passes it through untouched | **PARTIAL**: ADR-0018 stage 3 is **SERVED for Oscar** end-to-end as of run_59. The runner honors Oscar `mode: 'headless'` via the OscarDriver seam (refactor 6ff309e; honoring 67e7a99): fresh one-shot captured-subprocess invocations per dispatch reuse `runHeadlessProcess`, the file-artifact handshake stays unchanged, nudges are recorded but not delivered, and wrap-up pane delivery is skipped with `wrapup-delivery-skipped`. The Personas editor persists run-mode for Oscar only (7a0921e): `MODE_HONORED_PERSONAS`, display `runMode` derives from real `mode` instead of the old `enabled` conflation, Bob's toggle remains a local preview, and daemon-side `mode` pass-through is preserved. Still owed: Bob session honoring, gated on a captured-subprocess monitor path for builder work (the run_28 hang class). Evidence: core 216, daemon 130, UI 88, root typecheck clean |
