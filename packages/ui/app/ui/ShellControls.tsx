...
export function WorkspaceTabs({ workspaces, loadedIds, activeId, runsMap, onSelect, onClose, onLoad, onCreate }: {
...
export function ConnIndicator({ conn }: { conn: string }) {
...
export function OzGlobalControls({ theme, setTheme, conn, onRestartOz }: { theme: ShellTheme; setTheme: (fn: (t: ShellTheme) => ShellTheme) => void; conn: string; onRestartOz?: () => void }) {
  return (
    <>
      ...
      <button className="oz-iconbtn" title="Notifications" aria-label="Notifications"><Icon name="bell" size={15} /></button>
      <button className="oz-iconbtn" title="Toggle theme" aria-label="Toggle theme" onClick={() => setTheme((t) => (t === 'dark' ? 'light' : 'dark'))}><Icon name={theme === 'dark' ? 'sun' : 'moon'} size={15} /></button>
    </>
  )
}
