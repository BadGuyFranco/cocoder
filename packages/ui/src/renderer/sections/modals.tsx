...
export function CraftPersonaModal({ open, onClose, clis, onSubmit }: { open: boolean; onClose: () => void; clis: Cli[]; onSubmit: (p: { name: string; summary: string; placeAtTop: boolean }) => boolean | Promise<boolean> }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Craft a new persona" subtitle="Sketch the role. Oz files it as a workspace priority — the team builds the persona itself (prompt, Plays, tests)." icon="hammer" width={680}
      footer={<>
      ...
      <div style={{ padding: '12px 14px', background: 'var(--cb-bg)', border: '1px solid var(--cb-border)', borderRadius: 'var(--cb-radius-md)', marginBottom: 16 }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}><input type="checkbox" checked={needsSubAgents} onChange={(e) => setNeedsSubAgents(e.target.checked)} style={{ width: 14, height: 14, accentColor: 'var(--cb-accent)' }} /><span style={{ fontSize: 12.5, color: 'var(--cb-text)', fontWeight: 500 }}>This persona should use Plays</span></label>
        {needsSubAgents && <textarea className="oz-textarea" value={subAgentSketch} onChange={(e) => setSubAgentSketch(e.target.value)} placeholder="Sketch the Plays. e.g. 'a fact-checker Play on Gemini Pro; a formatter Play on Haiku.'" rows={2} style={{ marginTop: 10 }} />}
      </div>
