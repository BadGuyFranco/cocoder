...
async function request<T>(method: string, path: string, body?: unknown): Promise<DaemonResult<T>> {
  const r = await requestRaw<{ error?: string; code?: string; command?: string; runId?: string | null } & T>(method, path, body)
  if (!r.ok) return r
  if (r.status < 200 || r.status >= 300) {
    return {
      ...
      ...(typeof r.payload?.command === 'string' ? { command: r.payload.command } : {}),
