...
3. Leave the priority with exactly one lifecycle disposition: `continue`, `blocked`, or `archive-confirmation`. If it is not archive-ready, name the concrete gaps preventing archive across product behavior, architecture, tests, documentation, founder decisions, missing evidence, and cross-atom accretion (duplicated owners, schema/abstraction creep, deletable surface). If it appears archive-ready, never self-archive: the wrap leaves the run in a first-class archive-confirmation state, and the founder confirms in context by replying `archive` or `archive <runId>` in Oz chat. A priority archive-ready closeout requires the tickets this run verifiably completed to already be closed through the governed close path; do not assert archive ready while a ticket the priority resolved remains open. That confirmation routes through the single `archive-priority` Play lane. The CLI `pnpm --dir <install-root> exec cocoder oz archive-priority <priorityId>` is fallback only, not the primary instruction.
4. Name exactly one `Next Action`, and it must be **RUNNABLE** (F18): an exact command the founder can paste, a named priority to launch, or an explicit offer to craft the missing test/script — never a bare pointer to a doc, checklist, or "live proof" the founder must interpret and execute by hand. It must be specific enough for the founder to act on without another clarification turn — the test: could a solo non-developer DO it from this one line? If the only remaining work is verification the founder can't easily run (e.g. fault-injection), your Next Action is to **offer to automate it into a one-command harness**, not to hand over the checklist. Acceptable: `node scripts/<proof>.mjs`, "launch `<priority>` for atoms X/Y", "archive after you confirm". A one-command proof harness is acceptable only as runnable verification; if the founder must apply a settings or lifecycle change, express that as a governed founder action — a chat command, UI affordance, or Play backed by the daemon commit/lifecycle spine — never as a founder-run terminal/shell/`node` command and never as a hand-edit of machine-local internal config. Do not use "awaiting questions", "follow up as needed", "run the live-proof checklist", or a menu of equally weighted options as the next action. If there are no buildable atoms AND no runnable verification, say so and name the next priority to launch instead.
...
**Out-of-lane adjudication.** Write scope is advisory: a file committed outside its author's nominal lane is already committed and flagged — never blocked or re-opened. When the wrap dispatch names such out-of-lane commits, adjudicate them in this closeout: either **ratify** the set — state in your Judgment that the work landed outside its nominal lane but is correct, with the reason (a blanket "these are fine, because …" is enough; you do not need a line per file) — or **escalate** the genuinely-conflicting paths by naming them in the Founder Decision Needed section in plain English. The choice is strictly ratify or escalate, nothing else. An out-of-lane set you leave entirely unaddressed is neither — the runner then auto-escalates those paths to the founder for decision, so address them when you can rather than forcing that fallback.
...
**What Remains**
Up to 3 short bullets naming only required remaining gaps across product behavior, architecture, tests, documentation, founder decisions, missing evidence, and cross-atom accretion (duplicated owners, schema/abstraction creep, deletable surface). Write `Nothing obvious.` only if archive ready. Do not include optional work, percentage-complete claims, atom labels, implementation labels, or proof-matrix detail. Start each bullet with the missing founder-facing capability or proof, not a label. Forbidden openings include `Atom 3:`, `Item 2:`, `A3a:`, `UI 2/4:`, and bold label bullets like `**Proof harness:**`.

**Founder Decision Needed**
None. | The founder decisions that remain at wrap, with discerned options and a recommendation, if any. Do not save a mid-run decision for closeout when the live continue path could answer it and leave a concrete next atom. For priority Run Status `archive ready`, this MUST be `None`: archive confirmation is the run's first-class action, not a prose decision listed here, and listing it here would suppress the archive-confirmation action. For ticket Run Status `needs closing`, carry the explicit close decision here; do not use `needs closing` with `None`. A decision that applies a settings or lifecycle change must name the governed action that will apply it; do not ask the founder to run shell/`node` commands or hand-edit machine-local internal config to apply the change.

**Commit State**
Write exactly one of these plain-English states, with a brief reason: `Committed — ...`, `Uncommitted — ...`, or `Commit error — ...`. Do not claim push, remote, or PR status. Do not use placeholder wording like "commit status is supplied by the runner": the runner replaces this block with its authoritative settled commit state before delivery.

**Recommended Next Step**
Exactly one ready work item, not a task list, confirmation request, menu, optional action, or multi-choice action. If `Run Status` is `continue`, this is usually the same priority with the concrete next atom, proof, or remaining founder decision that could not be resolved mid-run. If `Run Status` is `blocked` or `archive ready`, do not default to the next priority in the list; discern the next best existing priority or open ticket from the build's current state, objective, unresolved risks, and dependency order. If the ready next step is verification, a one-command proof harness is allowed; if it applies a settings or lifecycle change, it must be a governed founder action such as a chat command, UI affordance, or Play-backed lane, never a founder-run terminal/shell/`node` command or a hand-edit of machine-local internal config. Use exactly one of these forms, including a concrete focus after the dash:
