...
3. Leave the priority with exactly one lifecycle disposition: `continue`, `blocked`, or `archive-confirmation`. If it is not archive-ready, name the concrete gaps preventing archive across product behavior, architecture, tests, documentation, founder decisions, and missing evidence. If it appears archive-ready, ask for founder archive confirmation; never self-archive. The runtime exposes archive confirmation as the post-wrap action; the founder should be able to confirm in the active Oscar/Oz context without translating the closeout into a separate file move or authoring recipe.
...
None. | The founder decisions that need to be made with discerned options and a recommendation, if any. For priority Run Status `archive ready`, ask the founder to confirm archive in this Oscar/Oz context; do not make a manual command or raw file move the primary path. The CLI may be named only as a fallback. For ticket Run Status `needs closing`, carry the explicit close decision here; do not use `needs closing` with `None`.
...
**Teardown Readiness**
Say the run is standing by and teardown requires an explicit founder request. For priority Run Status `archive ready`, also say the runtime is waiting for the archive confirmation action; if the active context cannot accept the confirmation, name the `cocoder oz archive-priority <priorityId>` CLI as a fallback only.
